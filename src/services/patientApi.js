...
function toPatient(apiPatient) {
    ...
    if (apiPatient.status === 'INACTIVE') formattedStatus = 'Inactive';
    if (apiPatient.status === 'BLOCKED') formattedStatus = 'Blocked';
...
export async function getPatients(page = 0, size = 50) {
    ...
    if (response && response.content) return response.content.map(toPatient);
    return [];
}
...
export async function replacePatient(patientId, patientData) {
    const payload = {
        ...
        email: patientData.email,
        identification: patientData.identification,
        status: (patientData.status || 'Active').toUpperCase() 
    };
